...
def plot_embedding_visualization(news_csv_path=None,
    ...
    if news_csv_path is None:
        news_csv_path = cfg.news_file
    if sbert_embeddings_path is None:
        sbert_embeddings_path = cfg.sbert_index_file
    if news_ids_path is None:
        news_ids_path = cfg.news_ids_file
    ...
    if save_path is None:
        save_path = Path(cfg.figures_dir) / "embedding_visualization.png"
...
def plot_embedding_by_category(sbert_embeddings_path=None,
    ...
    if save_path is None:
        save_path = Path(cfg.figures_dir) / "embedding_by_category.png"
    Path(save_path).parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    # 检查索引文件是否存在
    if not Path(cfg.sbert_index_file).exists():
        print(f"错误: SBERT向量文件不存在: {cfg.sbert_index_file}")
