...
class BasicTokenizer:
    ...
    def tokenize(self, text: str, never_split: Optional[List[str]] = None) -> List[str]:
        never_split = self.never_split.union(set(never_split or []))
        text = "".join(char for char in str(text) if ord(char) in (0, 0xFFFD) or not _is_control(char))
...
class WordpieceTokenizer:
    def __init__(self, vocab, unk_token="[UNK]", max_input_chars_per_word: int = 100) -> None:
        self.vocab = vocab
